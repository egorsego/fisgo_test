...
@Log4j
public class PaymentStepsDKF implements PaymentSteps{
    ...
    public PaymentStepsDKF(Manager manager, CashBox cashBox, Server server, WaitSteps wait, HubEmulSteps hub) {
    ...
    public void payOnePositionCard(int sum) {
        manager.pressKey(String.valueOf(sum));
        manager.pressKey(KeyEnum.keyEnter, 1);
        manager.pressKey(KeyEnum.keyPayByCard, 1);
        manager.pressKey(KeyEnum.keyEnter, 1);
        manager.sendCommands();
        manager.sleepPlease(2000);
        while (manager.getLoaderStatus()) {
