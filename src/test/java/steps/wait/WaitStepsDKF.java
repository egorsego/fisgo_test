...
public class WaitStepsDKF extends AbstractWaitSteps {

    public WaitStepsDKF(Manager manager, Server server) {
    ...
    public void waitExpectedEvent(Event event) {
        ...
        while (count != 60) {
            if (manager.getEventsContainer().getEventsMap().get(type).contains(event)) {
                break;
            }
    ...
    public void waitNotificationScreen(NotificationType type, String... text) {
        ArrayList<String> textArr = new ArrayList<>(Arrays.asList(text));
