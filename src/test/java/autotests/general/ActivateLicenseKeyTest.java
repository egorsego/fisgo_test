...
public class ActivateLicenseKeyTest extends BaseTestClass {
    ...
    private void checkActivateStatsOnValue(HubRequest requestStats) {
        ...
        if (cashBox.getBoxType().equals(CashBoxType.KASSA_F)) {
            ...
            assertEquals(services[1].getCode(), Integer.valueOf(4));
            assertEquals(services[1].getDescription(), "Продажа табачной продукции");

            assertEquals(services[2].getCode(), Integer.valueOf(3));
            assertEquals(services[2].getDescription(), "ФФД 1.05 с 1 июля 2019");
        }
    }
